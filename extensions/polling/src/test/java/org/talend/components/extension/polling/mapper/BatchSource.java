...
@Emitter(family = "test", name = "BatchSource")
public class BatchSource implements Serializable {
    ...
    public BatchSource(@Option("configuration") final Config config) {
        this.config = config;
    }
    ...
    public Data next() {
        if (!done) {
            ...
            return new Data("Data/" + config.getParam1(), config.getParam0() * 100);
        }
